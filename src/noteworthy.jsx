...
/* ═══════════════════════════════════════════════════════════════
   SPOTIFY CONFIG
   - Set your Client ID below
   - Add http://127.0.0.1:5173/ as a Redirect URI in Spotify dashboard
═══════════════════════════════════════════════════════════════ */
...
async function spSearch(q, types = "track,album", token) {
...
function Avatar({ user, size = 36 }) {
  const img = user?.images?.[0]?.url || user?.avatar_url;
  if (img) return (
    <img src={img} alt="" style={{ width: size, height: size, borderRadius: "50%", objectFit: "cover", flexShrink: 0, display: "block" }} />
  );
  const palette = ["#007AFF", "#34C759", "#FF9500", "#FF2D55", "#AF52DE", "#5AC8FA"];
  const name = user?.display_name || user?.name || "?";
  const bg = palette[(name.charCodeAt(0) || 0) % palette.length];
  return (
    <div style={{ width: size, height: size, borderRadius: "50%", background: bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.36, fontWeight: 700, color: "#fff", flexShrink: 0, fontFamily: "-apple-system, sans-serif" }}>
...
function CoverArt({ item, size = 52 }) {
  ...
  if (item?.cover) return (
    <img src={item.cover} alt={item.title} style={{ width: size, height: size, borderRadius: r, objectFit: "cover", flexShrink: 0, display: "block" }} />
  );
...
function RichEditor({ value, onChange, placeholder = "Write your review…" }) {
  ...
  const tools = [
    { label: <b>B</b>, title: "Bold", fn: () => exec("bold") },
    { label: <i>I</i>, title: "Italic", fn: () => exec("italic") },
    { label: <u style={{ textDecoration: "underline" }}>U</u>, title: "Underline", fn: () => exec("underline") },
    { label: "❝", title: "Blockquote", fn: () => exec("formatBlock", "blockquote") },
    { label: "•≡", title: "List", fn: () => exec("insertUnorderedList") },
    { label: "H", title: "Heading", fn: () => exec("formatBlock", "h3") },
    { label: "¶", title: "Paragraph", fn: () => exec("formatBlock", "p") },
    { label: "✕", title: "Clear formatting", fn: () => { exec("removeFormat"); exec("formatBlock", "p"); } },
  ];
  ...
  return (
    <div className="rich-wrap">
      <div className="rich-bar">
        {tools.map((t, i) => (
          <button key={i} className="rich-btn" title={t.title} onMouseDown={e => { e.preventDefault(); t.fn(); }}>{t.label}</button>
        ))}
      </div>
      <div ref={ref} className="rich-body" contentEditable suppressContentEditableWarning
        onInput={sync}
        onKeyDown={e => e.key === "Tab" && (e.preventDefault(), exec("insertHTML", "&nbsp;&nbsp;&nbsp;&nbsp;"))}
        data-ph={placeholder} />
    </div>
  );
}
...
/* ── Inline Search ────────────────────────────── */
function InlineSearch({ onSelect, placeholder = "Search songs or albums…", ensureToken }) {
  ...
  return (
    <div ref={wrap} className="isb-wrap">
      <div className="isb-field">
        <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="#666" strokeWidth="2" strokeLinecap="round">
...
/* ── Review Modal ─────────────────────────────── */
function ReviewModal({ initItem, ensureToken, onClose, onSubmit, initStars = 0, initText = "", isEdit = false }) {
        ...
        <div className="modal-body">
          {!item && !isEdit && (
            <>
              ...
              <InlineSearch ensureToken={ensureToken} onSelect={setItem} placeholder="Search Spotify…" />
...
/* ── List Modal ───────────────────────────────── */
function ListModal({ ensureToken, onClose, onSubmit, initList = null, isEdit = false }) {
    ...
    <div className="modal-ov" onClick={onClose}>
      <div className="modal-box modal-box-lg" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <InlineSearch ensureToken={ensureToken} onSelect={add} placeholder="Search to add…" />
          {items.length > 0 && (
...
function Alert({ title, message, actions, onClose }) {
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", animation: "fadeIn .15s ease" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#2C2C2E", borderRadius: 16, width: 280, overflow: "hidden", animation: "popIn .2s cubic-bezier(.34,1.56,.64,1)" }}>
        ...
        <div style={{ borderTop: "1px solid #3A3A3C", display: "flex" }}>
          {actions.map((a, i) => (
            <button key={i} onClick={a.action} style={{ flex: 1, padding: "14px 8px", background: "none", border: "none", borderLeft: i > 0 ? "1px solid #3A3A3C" : "none", color: a.destructive ? "#FF3B30" : a.primary ? "#007AFF" : "#8E8E93", fontSize: 17, fontWeight: a.primary ? 600 : 400, cursor: "pointer", fontFamily: "-apple-system, sans-serif" }}>
              {a.label}
            </button>
          ))}
        </div>
  ...
  return (
    <article className="rc">
      <div className="rc-art" onClick={() => navigate("reviewDetail", { reviewId: review.id })}>
        {review.item.cover
          ? <img src={review.item.cover} alt={review.item.title} className="rc-art-img" />
          : <div className="rc-art-bg" style={{ "--h": ((review.item.title || "").charCodeAt(0) * 23) % 360 }} />
        }
      ...
      <div className="rc-body">
        ...
        {review.text && (
          <div className="rc-snippet" onClick={() => navigate("reviewDetail", { reviewId: review.id })}
            dangerouslySetInnerHTML={{ __html: review.text }} />
        )}
        <div className="rc-actions">
          <button className={`rc-act ${liked ? "liked" : ""}`} onClick={() => toggleLike(review.id)}>
            <svg viewBox="0 0 24 24" width="13" height="13" fill={liked ? "#E9463F" : "none"} stroke={liked ? "#E9463F" : "#555"} strokeWidth="2" strokeLinecap="round">
          ...
          <button className="rc-act" onClick={() => navigate("reviewDetail", { reviewId: review.id })}>
            <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="#555" strokeWidth="2" strokeLinecap="round">
...
export default function Noteworthy() {
  ...
  const [refreshToken, setRefreshToken] = useState(() => lsGet(LS.REFRESH));
  const [tokenExp, setTokenExp] = useState(() => lsGet(LS.EXP, 0));
  ...
  // Refs are seeded DIRECTLY from localStorage so they are valid before
  // any useEffect fires — this is the key fix for the 401 race condition.
  const tokenRef = useRef(() => {
    const t = lsGet(LS.TOKEN);
    const exp = lsGet(LS.EXP, 0);
    return (t && Date.now() < exp - 60000) ? t : null;
  });
  const refreshTokenRef = useRef(lsGet(LS.REFRESH));
  const tokenExpRef = useRef(lsGet(LS.EXP, 0));

  // Initialise tokenRef.current by calling the factory if needed
  if (typeof tokenRef.current === "function") {
    tokenRef.current = tokenRef.current();
  }

  // Keep refs in sync whenever state updates
  useEffect(() => { tokenRef.current = token; }, [token]);
  useEffect(() => { refreshTokenRef.current = refreshToken; }, [refreshToken]);
  useEffect(() => { tokenExpRef.current = tokenExp; }, [tokenExp]);

  /* ── ensureToken ─────────────────────────────────────────────
     Always call this before any Spotify API request.
     Reads directly from refs (never stale state), and refreshes
     the token transparently when it is expired or missing.
  ──────────────────────────────────────────────────────────── */
  const ensureToken = async () => {
    const now = Date.now();
    const currentToken = tokenRef.current;
    const currentExp = tokenExpRef.current;
    const currentRefresh = refreshTokenRef.current;

    // Happy path — valid token in memory
    if (currentToken && now < currentExp - 60000) {
      return currentToken;
    }

    // Token missing or expired — try silent refresh
    if (currentRefresh) {
      const data = await refreshAccessToken(currentRefresh);
      if (data?.access_token) {
        const exp = now + data.expires_in * 1000;
        // Update both refs and state so everything stays in sync
        ...
        if (data.refresh_token) {
          ...
          lsSet(LS.REFRESH, data.refresh_token);
        }
        return data.access_token;
      }
    }

    // Nothing worked — clear session
    signOut();
    return null;
  };

  /* ── On mount: restore token from storage or refresh it ── */
  useEffect(() => {
    const storedToken = lsGet(LS.TOKEN);
    const storedExp = lsGet(LS.EXP, 0);
    const storedRefresh = lsGet(LS.REFRESH);

    if (storedToken && Date.now() < storedExp - 60000) {
      // Token still valid — update both state AND ref immediately
      tokenRef.current = storedToken;
      tokenExpRef.current = storedExp;
      setToken(storedToken);
    } else if (storedRefresh) {
      // Token expired or missing — refresh silently
      setAuthLoading(true);
      refreshAccessToken(storedRefresh).then(data => {
        if (data?.access_token) {
          const exp = Date.now() + data.expires_in * 1000;
          // Update refs first so any in-flight search picks them up immediately
          tokenRef.current = data.access_token;
          tokenExpRef.current = exp;
  ...
  useEffect(() => {
    ...
    exchangeCodeForToken(code).then(async data => {
      if (!data?.access_token) { setAuthLoading(false); return; }
      const exp = Date.now() + data.expires_in * 1000;
      // Update refs immediately so ensureToken works right away
      tokenRef.current = data.access_token;
      tokenExpRef.current = exp;
      if (data.refresh_token) refreshTokenRef.current = data.refresh_token;
      setToken(data.access_token);
  ...
  function signOut() {
    lsClear(LS.TOKEN, LS.REFRESH, LS.EXP, LS.PROFILE);
    setToken(null); setRefreshToken(null); setProfile(null); setTokenExp(0);
  ...
  function editReview(id, stars, text) {
    ...
    setModal(null); notify("Review updated");
  }
  ...
  function addComment(id, text) {
    setReviews(p => p.map(r => r.id !== id ? r : {
      ...r, comments: [...(r.comments || []), { id: "cm" + Date.now(), userId: myId, text, createdAt: new Date().toISOString() }]
    }));
  }
  ...
  function toggleFollow(targetId, targetProfile) {
    if (!myId) return;
    const current = follows[myId] || [];
    const has = current.includes(targetId);
    const updated = has ? current.filter(x => x !== targetId) : [...current, targetId];
    setFollows(prev => ({ ...prev, [myId]: updated }));
    if (targetProfile) {
      setUsersCache(prev => { const n = { ...prev, [targetId]: targetProfile }; lsSet(LS.USERS, n); return n; });
    }
  ...
  function editList(id, title, desc, items) {
    ...
    setModal(null); notify("List updated");
  }
  ...
  const shared = {
    ...
    ensureToken, navigate, toggleLike, addComment, deleteComment,
        ...
        <div className="landing">
          <div className="landing-inner">
            <div className="landing-logo">
              <span className="landing-note">♪</span>
              <span className="landing-wordmark">noteworthy</span>
            </div>
      ...
      <>
        ...
        <div className="landing">
          <div className="landing-inner">
            ...
            <div className="auth-loading"><div className="auth-spin" /><span>Signing you in…</span></div>
          </div>
        </div>
      </>
    );
    ...
    <>
      ...
      <div className="nw-app">
        ...
        <div className="nw-main">
          {tab === "feed" && <FeedTab     {...shared} signOut={signOut} />}
      ...
      {modal?.type === "newReview" && <ReviewModal ensureToken={ensureToken} onClose={() => setModal(null)} onSubmit={addReview} />}
      {modal?.type === "editReview" && <ReviewModal ensureToken={ensureToken} onClose={() => setModal(null)} onSubmit={(_, s, t) => editReview(modal.review.id, s, t)} initItem={modal.review.item} initStars={modal.review.stars} initText={modal.review.text} isEdit />}
      {modal?.type === "newList" && <ListModal ensureToken={ensureToken} onClose={() => setModal(null)} onSubmit={addList} />}
      {modal?.type === "editList" && <ListModal ensureToken={ensureToken} onClose={() => setModal(null)} onSubmit={(t, d, i) => editList(modal.list.id, t, d, i)} initList={modal.list} isEdit />}
      {modal?.type === "newReviewWithItem" && <ReviewModal ensureToken={ensureToken} initItem={modal.item} onClose={() => setModal(null)} onSubmit={addReview} />}
...
function RecentlyPlayed({ ensureToken, onReview, title = "Your Recent Listening" }) {
  ...
  if (loading) return (
    <div className="rp-wrap">
      ...
      <div style={{ display: "flex", gap: 10, padding: "8px 0 4px", overflowX: "hidden" }}>
          ...
          return (
            <div key={track.id + played_at} className="rp-card" onClick={() => onReview(item)}>
              <div className="rp-art-wrap">
                {item.cover
                  ? <img src={item.cover} alt={item.title} className="rp-art" />
                  : <div className="rp-art-fallback" style={{ "--h": (item.title.charCodeAt(0) * 23) % 360 }} />
                }
...
/* ── Feed Tab ─────────────────────────────────── */
function FeedTab({ reviews, allUsers, currentUser, myId, myFollowing, ensureToken, toggleLike, navigate, setModal, deleteReview, signOut }) {
  ...
  return (
    <div className="screen">
      ...
      <div className="sticky-search">
        <InlineSearch ensureToken={ensureToken} onSelect={item => setModal({ type: "newReviewWithItem", item })} placeholder="Search to review a song or album…" />
      </div>
        ...
        {feed.length === 0 ? (
          <>
            <RecentlyPlayed
              ...
              onReview={item => setModal({ type: "newReviewWithItem", item })}
...
/* ── Discover Tab ─────────────────────────────── */
function DiscoverTab({ reviews, allUsers, currentUser, myId, myFollowing, ensureToken, toggleLike, navigate, setModal, deleteReview, toggleFollow, follows }) {
  ...
  const peopleResults = peopleQ.trim()
    ? allKnownUsers.filter(u => (u.display_name || "").toLowerCase().includes(peopleQ.toLowerCase()) || (u.id || "").toLowerCase().includes(peopleQ.toLowerCase()))
    : allKnownUsers;
        ...
        {mainTab === "music" ? (
          <div className="isb-wrap">
            <div className="isb-field">
              ...
              <input className="isb-input" placeholder="Search Spotify to review…"
...
function ActivityTab({ reviews, allUsers, currentUser, myId, myFollowing, toggleLike, navigate, setModal, deleteReview }) {
  const activity = reviews
    .filter(r => myFollowing.includes(r.userId))
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  return (
    <div className="screen">
...
function ListCard({ list, allUsers, currentUser, myId, navigate, setModal, setAlert, deleteList }) {
  const author = allUsers[list.userId];
  const isOwn = list.userId === myId;
  const [menuOpen, setMenuOpen] = useState(false);
  const menuRef = useRef(null);
  ...
  return (
    <div className="list-card" onClick={() => navigate("listDetail", { listId: list.id })}>
      <div className="lc-top">
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="lc-title">{list.title}</div>
          {list.description && <div className="lc-desc">{list.description}</div>}
        </div>
        {isOwn && (
          <div ref={menuRef} style={{ position: "relative" }} onClick={e => e.stopPropagation()}>
            <button className="rc-menu-btn" onClick={() => setMenuOpen(v => !v)}>
              <svg viewBox="0 0 24 24" width="15" height="15" fill="none"><circle cx="5" cy="12" r="1.4" fill="#555" /><circle cx="12" cy="12" r="1.4" fill="#555" /><circle cx="19" cy="12" r="1.4" fill="#555" /></svg>
            </button>
            {menuOpen && (
              <div className="rc-menu-drop">
                <button onClick={() => { setMenuOpen(false); setModal({ type: "editList", list }); }}>Edit</button>
                <button className="danger" onClick={() => { setMenuOpen(false); deleteList(list.id); }}>Delete</button>
              </div>
            )}
          </div>
        )}
      ...
      <div className="lc-items">
        {list.items.slice(0, 4).map((item, i) => (
          <div key={item.id} className="lc-row">
            <span className="lc-rank">{i + 1}</span>
            <CoverArt item={item} size={34} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="lc-iname">{item.title}</div>
              <div className="lc-imeta">{item.artist}</div>
            </div>
          </div>
        ))}
      ...
      {author && (
        <div className="lc-foot">
          ...
          <span>{author.display_name || author.id} · {list.items.length} items</span>
        </div>
      )}
    </div>
  );
...
function MyProfileTab(props) {
  ...
  const { currentUser, myId, reviews, lists, allUsers, toggleLike, navigate, setModal, setAlert, deleteReview, deleteList, follows, ensureToken } = props;
  ...
  return (
    <div className="screen">
      <header className="nw-header" style={{ borderBottom: "none" }}>
        <span className="nw-logo" style={{ fontSize: 18 }}>{currentUser?.display_name}</span>
      </header>
      <div className="scroll-area">
        ...
        {tab === "reviews" && (myReviews.length === 0
          ? <div className="empty-feed"><div className="empty-art">⭐</div><h2 className="empty-h">No reviews yet</h2><button className="empty-cta" onClick={() => setModal({ type: "newReview" })}>Write your first review</button></div>
          : myReviews.map(r => <ReviewCard key={r.id} review={r} currentUser={currentUser} allUsers={allUsers} toggleLike={toggleLike} navigate={navigate} onEdit={rev => setModal({ type: "editReview", review: rev })} onDelete={id => deleteReview(id)} />)
        )}
        {tab === "lists" && (myLists.length === 0
          ? <div className="empty-feed"><div className="empty-art">📋</div><h2 className="empty-h">No lists yet</h2></div>
          : myLists.map(l => <ListCard key={l.id} list={l} allUsers={allUsers} currentUser={currentUser} myId={myId} navigate={navigate} setModal={setModal} setAlert={setAlert} deleteList={deleteList} />)
        )}
      </div>
    </div>
  );
  ...
  if (!user) return (
    <div className="screen">
      <header className="nw-header">
        ...
        <span className="nw-logo" style={{ fontSize: 17 }}>Profile</span>
        <div style={{ width: 32 }} />
      </header>
  ...
  return (
    <div className="screen">
      ...
      <div className="scroll-area">
        ...
        {userId !== myId && (
          <button className={`profile-follow-btn ${isFollowing ? "following" : ""}`} onClick={() => toggleFollow(userId, user)}>
            {isFollowing ? "Following" : "Follow"}
          </button>
        )}
        <div className="seg-ctrl">
        ...
        {tab === "reviews" && (userReviews.length === 0
          ...
          : userReviews.map(r => <ReviewCard key={r.id} review={r} currentUser={currentUser} allUsers={allUsers} toggleLike={toggleLike} navigate={navigate} onEdit={rev => setModal({ type: "editReview", review: rev })} onDelete={id => deleteReview(id)} />)
        )}
        {tab === "lists" && (userLists.length === 0
          ? <div className="empty-inline">No lists yet.</div>
          : userLists.map(l => <ListCard key={l.id} list={l} allUsers={allUsers} currentUser={currentUser} myId={myId} navigate={navigate} setModal={setModal} setAlert={setAlert} deleteList={deleteList} />)
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="screen">
      <header className="nw-header">
        ...
        {live.userId === myId && (
          <button className="rc-menu-btn" onClick={() => setAlert({
            title: null, message: null, actions: [
              { label: "Edit", action: () => { setAlert(null); setModal({ type: "editReview", review: live }); } },
              { label: "Delete", destructive: true, action: () => { setAlert(null); deleteReview(live.id); goBack(); } },
              { label: "Cancel", action: () => setAlert(null) },
            ]
          })}>
            <svg viewBox="0 0 24 24" width="18" height="18" fill="none"><circle cx="5" cy="12" r="1.6" fill="#555" /><circle cx="12" cy="12" r="1.6" fill="#555" /><circle cx="19" cy="12" r="1.6" fill="#555" /></svg>
          </button>
        )}
      </header>
      <div className="scroll-area">
        ...
        {author && (
          <button className="rd-author" onClick={() => navigate("profile", { userId: author.id })}>
            <Avatar user={author} size={28} />
            <div>
              <div style={{ fontSize: 14, fontWeight: 600, color: "#fff" }}>{author.display_name || author.id}</div>
              <div style={{ fontSize: 12, color: "#555" }}>{formatDate(live.createdAt)}</div>
            </div>
          </button>
        )}
          ...
          {(live.comments || []).map(c => {
            ...
            return (
              <div key={c.id} className="comment-row">
                ...
                <div style={{ flex: 1 }}>
                  <span className="comment-author">{cu2?.display_name || c.userId} </span>
        ...
        <div className="comment-compose">
          ...
          <input className="comment-in" placeholder="Add a comment…" value={text} onChange={e => setText(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter" && text.trim()) { addComment(live.id, text.trim()); setText(""); } }} />
    ...
    <div className="screen">
      <header className="nw-header">
        ...
        {isOwn && (
          <button className="rc-menu-btn" onClick={() => setAlert({
            title: null, message: null, actions: [
              { label: "Edit List", action: () => { setAlert(null); setModal({ type: "editList", list: live }); } },
              { label: "Delete List", destructive: true, action: () => { setAlert(null); deleteList(live.id); goBack(); } },
              { label: "Cancel", action: () => setAlert(null) },
            ]
          })}>
            <svg viewBox="0 0 24 24" width="18" height="18" fill="none"><circle cx="5" cy="12" r="1.6" fill="#555" /><circle cx="12" cy="12" r="1.6" fill="#555" /><circle cx="19" cy="12" r="1.6" fill="#555" /></svg>
      ...
      <div className="scroll-area">
        <div className="ld-head">
          <div className="ld-title">{live.title}</div>
          {live.description && <div className="ld-desc">{live.description}</div>}
          {author && (
            <button className="rd-author" style={{ marginTop: 8 }} onClick={() => navigate("profile", { userId: author.id })}>
              <Avatar user={author} size={22} />
              <span style={{ fontSize: 13, color: "#555" }}>{author.display_name || author.id} · {live.items.length} items</span>
            </button>
          )}
        </div>
        {live.items.map((item, i) => (
          <div key={item.id} className="ld-row">
            <div className="ld-rank">{i + 1}</div>
            <CoverArt item={item} size={46} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: "#e8e8e8" }}>{item.title}</div>
              <div style={{ fontSize: 12, color: "#555" }}>{item.artist} · {item.type}</div>
            </div>
          </div>
        ))}
      </div>
...
:root {
  ...
  --red:     #E9463F;
  --blue:    #40bcf4;
  --font:    'Titillium Web', -apple-system, sans-serif;
...
.rd-body strong { color:var(--text); font-weight:600; }
...
/* Misc */
.spin-row { display:flex; justify-content:center; padding:16px; }
.mini-spin { width:18px; height:18px; border:2px solid var(--s3); border-top-color:var(--blue); border-radius:50%; animation:spin .7s linear infinite; }
